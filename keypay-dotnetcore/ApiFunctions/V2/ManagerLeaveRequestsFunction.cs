...
namespace KeyPay.ApiFunctions.V2
{
    public class ManagerLeaveRequestsFunction : BaseFunction
    {
        ...
        {
            return ApiRequest<List<ManagerLeaveRequestModel>>(
                $"/business/{businessId}/manager/leaverequest/overlapping?fromDate={fromDate:yyyy-MM-dd}&toDate={toDate:yyyy-MM-dd}");
        }
        ...
        public List<LeaveBalanceModel> Balances(int businessId, int employeeId, DateTime? asAtDate)
        {
            var queryString = asAtDate.HasValue
                ? $"?asAtDate={asAtDate.Value:yyyy-MM-dd}"
                : "";
        ...
        {
            return ApiRequest<ManagerLeaveEstimateModel>(
                $"/business/{businessId}/manager/{employeeId}/leaverequest/estimate?fromDate={fromDate:yyyy-MM-dd}&toDate={toDate:yyyy-MM-dd}&leaveCategoryId={leaveCategoryId}");
        }
